...
void setup()
{
  ...
  digitalWrite(ledPin2, LOW); //start LED off
...
void selectProtocol()
{
  ...
  // startup stick commands
  set_txid(true);                      // Renew Transmitter ID
  current_protocol = PROTO_E010; // PROTO_MJX; // PROTO_E010; // PROTO_E010;  //PROTO_MJX;//PROTO_E010;  // Cheerson CX10(blue pcb, newer red pcb)/CX10-A/0CX11/CX12 ...
  // update eeprom
  EEPROM.update(ee_PROTOCOL_ID, current_protocol);
}
...
void init_protocol()
{
  switch (current_protocol) {
    ...
    case PROTO_E010:
      MJX_init();
      MJX_bind();
      Serial.println("PROTO_E010-PROTO_MJX initialized and bound");
      break;
    case PROTO_HISKY:
      HiSky_init();
      break;
